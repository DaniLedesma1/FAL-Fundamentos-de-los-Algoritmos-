...
void particion (std::vector<psi> & v, int &p, int &q) {
	...
	int contini = 0, contfin = v.size() - 1;
	int contverdes = 0;
	while(contverdes <= contfin ){
		
		if (v[contverdes].first == 'v') {
			contverdes++;
		}
		else if (v[contverdes].first == 'a') {
			swap(v[contini], v[contverdes]);
			contini++;
			contverdes++;
		}
		else {
			swap(v[contverdes], v[contfin]);
			contfin--;
		}
	}
